...
export default function Hero() {
  return (
    <div className="flex flex-col items-center justify-center max-w-[50%] gap-8 mt-8">
      <div className="flex flex-row items-center justify-center space-x-2 bg-brown/10 py-2 px-4 rounded-full">
        <div className="flex flex-row -space-x-1">
          {Array.from({ length: 4 }).map((_, index) => (
            <Avatar
              key={index}
              className="hover:-translate-y-2 transition-transform duration-300 w-8 h-8 ring ring-white"
            >
      ...
      <h1 className="font-serif text-4xl text-center text-brown">
      ...
      <Button className="bg-orange-800/90 hover:bg-orange-800 rounded-full">
        Hemen Başlayın
      </Button>
    </div>
  );
